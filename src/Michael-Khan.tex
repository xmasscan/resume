...
\begin{header}
	...
	\normalsize
	\mbox{{\color{black}{\footnotesize\faEnvelope[regular]}\hspace*{0.13cm}gmail: michaelk30200389}}%
	\kern 0.25 cm%
	\AND%
	\kern 0.25 cm%
...
\vspace{0.10 cm}
\begin{onecolentry}
	\textbf{Coursework:} Data Structures, Software Design, Systems Programming, Algorithms, Principles of Concurrency
\end{onecolentry}

\section{Experience}

\textbf{Discovery Partners Institute (DPI)}
\begin{twocolentry}{
    \textit{Fall 2025 - Current}
    }
    \textbf{Lab Research Assistant}
\end{twocolentry}
\begin{onecolentry}
    \begin{highlights}
      \item Deployed automatic endpoint fuzzing instrumentation responsible for performing thousands of parameter-specific edge case tests per day over multiple weeks. 
      \item Orchestrated dynamically generated Replay and Historian Compromise attacks to occur in tandem between an attacker and faux client fuzzer.
      \item Extended existing message generation infrastructure in tandem with a custom-designed Payload Injector and orchestration code to emulate Payload Injection attacks and Replay Attacks. 
    \end{highlights}
\end{onecolentry}
...
\begin{onecolentry}
  \begin{itemize}
    \item Revitalized a dormant Student Org by expanding the executive board and compiling a body of approximately 20 dedicated volunteers.
    \item Led multiple inter-organizational collaborations to promote Cybersecurity education alongside UIC's ACM and WiCyS chapters.
    \item Actively manages and supports our organization's dedicated CTF teams in both training and competition participation.
  \end{itemize}
\end{onecolentry}

\begin{onecolentry}
  \textbf{Events:} Intro to Web Security: SQL Injections \& XSS, Linux Week, Address Sanitizer Workshop, Intro to ViM, Intro to PC Building
\end{onecolentry}
...
\begin{onecolentry}
  ...
  Rigel Gjomemo, Ronald Varriale, Soham Gumaste, \& Michael Khan (2026, May 20th). EVSEFuzz: A Tool for Automated Testing of Protocol Implementations. Paper to be presented at ESCAR, the Embedded Security in Cars conference in Novi, Michigan. 
\end{onecolentry}
...
\begin{onecolentry}
  \textbf{Skills:} Network Security, Network Forensics, Digital Forensics, Binary Analysis, Vulnerability Documentation, Attack Emulation, Fuzzing
\end{onecolentry}
